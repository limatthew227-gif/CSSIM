...
OUT_JSON = ROOT / "data" / "hltv-player-splits-uc-2026-06-08.json"
OUT_MD = ROOT / "data" / "hltv-player-splits-uc-2026-06-08.md"
OUT_TS = ROOT / "src" / "hltvPlayerSplits2026.ts"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Scrape HLTV player split rankings with Scrapy selectors and undetected-chromedriver.")
    parser.add_argument("--headless", action="store_true", help="Use headless Chrome. Visible Chrome is more likely to clear HLTV's managed challenge.")
    parser.add_argument("--write-ts", action="store_true", help="Write src/hltvPlayerSplits2026.ts with matched samples.")
    parser.add_argument("--limit", type=int, default=None, help="Limit expected players for debugging.")
    parser.add_argument("--sleep", type=float, default=1.5, help="Delay between HLTV ranking pages.")
    ...
    report = {
        ...
        "method": "scrapy-selector + undetected-chromedriver",
        ...
        "notes": [
            "Uses undetected-chromedriver to load HLTV's stats ranking pages and Scrapy selectors to parse player rows.",
            "The scrape uses minMapCount=0 so low-sample opposition filters are preserved instead of disappearing from the table.",
            "Values are Rating 3.0 from HLTV's player ranking table for the January 1, 2026 through June 8, 2026 window.",
            "Placeholder fifths from the ranking/team-page audit are matched by handle and team when they appear in HLTV's stats ranking table.",
        ],
        "players": players,
...
def build_ranking_url(ranking_filter: str | None) -> str:
    query: dict[str, str] = {"startDate": START_DATE, "endDate": END_DATE, "minMapCount": "0"}
    if ranking_filter:
...
def render_markdown(report: dict[str, Any]) -> str:
    lines = [
        "# HLTV Player Split Scrape UC - 2026-06-08",
        "",
    ...
    lines.extend(["", "## Player Splits", "", "| Team | Player | Overall | Top5 | Top10 | Top20 | Top50 |", "| --- | --- | --- | --- | --- | --- | --- |"])
